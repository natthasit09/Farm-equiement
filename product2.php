...
switch($_GET["action"]) {
	case "add":
		if(!empty($_POST["quantity"])) {
			$productByCode = $db_handle->runQuery("SELECT * FROM products_2 WHERE code='" . $_GET["code"] . "'");
...
<!-- ปุ่มแอดสินค้า -->
<div id="product-grid">
	<div class="txt-heading">ประเภทดิน</div>
	<a id="type-product"  href="product1.php">ปุ๋ย</a>
    <a id="type-product"  href="product3.php">ประเภทพืช</a>
	
	<?php
	$product_array = $db_handle->runQuery("SELECT * FROM products_2 ORDER BY id ASC");
	if (!empty($product_array)) { 
		foreach($product_array as $key=>$value){
	?>
		<div class="product-item">
			<form method="post" action="product2.php?action=add&code=<?php echo $product_array[$key]["code"]; ?>">
